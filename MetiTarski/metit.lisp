...
(defun init-metit (prebins arch)
  (let* ((metit-lib)
    ...
    (cond ((or (null metit-bin) (not (probe-file metit-bin)))
	   (format t "~%metit's executable file (~a) not found.~%" metit-bin))
	  ((or (null z3-bin) (not (probe-file z3-bin)))
	   (format t "~%z3's executable file (~a) not found. Set environment variable Z3_NONLIN~%" z3-bin))
	  ((not (uiop:directory-exists-p metit-tptp))
	   (format t "~%metit's tptp directory (~a) not found. Set environment variable TPTP~%" metit-tptp))
	  (t 
	   (setq *z3-bin* z3-bin)
