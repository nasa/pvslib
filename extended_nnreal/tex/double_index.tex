...
\begin{alltt}
\pvsid{double\_index}\({\pvsbracketl}\)\(T\): \pvskey{TYPE}\({\pvsbracketr}\): \pvskey{THEORY}
 \pvskey{BEGIN}

  \pvskey{IMPORTING} \pvsid{code\_product}

  \(i\), \(j\), \(n\): \pvskey{VAR} \(\mathbb{N}\)\vspace*{\pvsdeclspacing}

  \(u\): \pvskey{VAR} \({\pvsbracketl}\)\({\pvsbracketl}\)\(\mathbb{N}\), \(\mathbb{N}\)\({\pvsbracketr}\) \(\rightarrow\) \(T\)\({\pvsbracketr}\)\vspace*{\pvsdeclspacing}

  \(v\): \pvskey{VAR} \({\pvsbracketl}\)\(\mathbb{N}\) \(\rightarrow\) \(T\)\({\pvsbracketr}\)\vspace*{\pvsdeclspacing}

  \pvsid{single\_index}\pvsid{(}\(u\)\pvsid{)}: \({\pvsbracketl}\)\(\mathbb{N}\) \(\rightarrow\) \(T\)\({\pvsbracketr}\) \pvskey{=}
      \pvsid{(}\(\lambda\) \(n\): \(u\)\pvsid{(}\pvsid{double\_index\_i}\pvsid{(}\(n\)\pvsid{)}, \pvsid{double\_index\_j}\pvsid{(}\(n\)\pvsid{)}\pvsid{)}\pvsid{)}\vspace*{\pvsdeclspacing}

  \pvsid{double\_index}\pvsid{(}\(v\)\pvsid{)}: \({\pvsbracketl}\)\({\pvsbracketl}\)\(\mathbb{N}\), \(\mathbb{N}\)\({\pvsbracketr}\) \(\rightarrow\) \(T\)\({\pvsbracketr}\) \pvskey{=} \(\opohtwofn{v}{\pvsid{double\_index\_n}}\)\vspace*{\pvsdeclspacing}

  \pvsid{single\_double}: \pvskey{LEMMA} \pvsid{single\_index}\pvsid{(}\pvsid{double\_index}\pvsid{(}\(v\)\pvsid{)}\pvsid{)} \(=\) \(v\)\vspace*{\pvsdeclspacing}

  \pvsid{double\_single}: \pvskey{LEMMA} \pvsid{double\_index}\pvsid{(}\pvsid{single\_index}\pvsid{(}\(u\)\pvsid{)}\pvsid{)} \(=\) \(u\)\vspace*{\pvsdeclspacing}

 \pvskey{END} \pvsid{double\_index}\end{alltt}
